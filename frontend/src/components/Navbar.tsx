...
export function Navbar() {
  ...
  return (
    <nav className="relative z-50 shrink-0 border-b border-slate-800/80 bg-slate-950/80 shadow-2xl shadow-black/20 backdrop-blur-xl">
      <div className="mx-auto flex h-16 w-full max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
        <a href="/" className="flex items-center gap-3 text-white">
          <span className="grid size-9 place-items-center rounded-xl border border-amber-300/35 bg-gradient-to-br from-amber-300 to-amber-600 text-slate-950 shadow-lg shadow-amber-500/20">
            <Dices className="size-5" strokeWidth={2.5} />
          </span>
          <span className="font-display text-sm font-black tracking-tight sm:text-base">
            <span className="text-amber-300">ONLINE</span> BOARD GAMES
          </span>
        </a>
        ...
        {isLoading ? (
          ...
          <div className="relative">
            <button
              ...
              className="flex items-center gap-3 rounded-full border border-slate-700/70 bg-slate-900/80 p-1.5 pr-3 text-left shadow-lg shadow-black/10 transition-all hover:border-amber-400/35 hover:bg-slate-800/90"
            >
              <span className="grid size-9 place-items-center rounded-full border border-amber-200/25 bg-gradient-to-br from-amber-300 to-orange-600 text-xs font-black text-slate-950">{initials}</span>
              <span className="hidden sm:block">
                <span className="block max-w-36 truncate text-sm font-bold text-slate-100">{user.display_name}</span>
                <span className={isGuest ? "mt-0.5 inline-flex rounded-full bg-amber-400/10 px-2 py-0.5 text-[9px] font-black tracking-wider text-amber-300 uppercase" : "mt-0.5 inline-flex rounded-full bg-emerald-400/10 px-2 py-0.5 text-[9px] font-black tracking-wider text-emerald-300 uppercase"}>{user.is_admin ? "Admin" : isGuest ? "Guest" : "Member"}</span>
              </span>
